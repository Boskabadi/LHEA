function EKF_CSTR_L21(block)
% L2 EKF (bias-resistant, no ML)
% Uses measurements [V, CX, DO] fast + MEV assay every 12 h
%
% Internal augmented state:
%   x_aug = [V; CX; LAC; N; MEV; DO; bV; bDO]
%
% InputPort(1): x_true (6)  (used only for trajectory metrics)
% InputPort(2): y_meas (4)  = [V; CX; DO; MEV_assay]
% InputPort(3): dt (1)      = hours
%
% OutputPort(1): xhat_phys (6)
%
% IMPORTANT:
% This code assumes Simulink model time is in HOURS.
...
function setup(block)
    ...
    % Keep external output compatible with the original 6-state estimator
...
function PostPropSetup(block)
    ...
    block.Dwork(1).Name = 'xhat';                block.Dwork(1).Dimensions = 8;
    block.Dwork(2).Name = 'P';                   block.Dwork(2).Dimensions = 64;
...
function InitConditions(block)
    ...
    % Physical-state covariance
    P_phys = diag([1e2 5 5 5 5 1e-4]);

    % Bias covariance:
    % V bias can be relatively large; DO bias is much smaller in magnitude
    P_bias = diag([50^2, (5e-3)^2]);
    ...
    assignin('base', 'L1_assay_log', zeros(0,10));
    assignin('base', 'L1_assay_log_cols', ...
        {'t_h','y_mev','xpred_mev','xpost_mev','innov_mev', ...
         'P55_pred','K55_mev','nis','bV_hat','bDO_hat'});
    assignin('base', 'L1_assay_metrics', [NaN NaN NaN NaN]);  % [RMSE MAE R2 NIS_cov]
    assignin('base', 'L1_traj_metrics',  [NaN NaN NaN]);      % [RMSE MAE R2]
    assignin('base', 'L1_bias_est',      [0 0]);              % [bV bDO]
end

function Output(block)
    % Output only the physical 6-state estimate for compatibility
    xhat_aug = block.Dwork(1).Data;
    block.OutputPort(1).Data = xhat_aug(1:6);
end
...
function Update(block)
    ...
    xhat_aug = block.Dwork(1).Data;        % 8x1
    ...
    % ---------- process noise ----------
    % Continuous-time intensities, then scaled by dt
    Qc_phys = diag([1e-2 1e-3 1e-3 1e-4 1e-4 1e-6]);

    % Bias random-walk intensities:
    % tune these if the bias adapts too slowly or too aggressively
    sigma_bV_rw  = 2.0;      % (units of V)/sqrt(h)
    sigma_bDO_rw = 2e-4;     % (units of DO)/sqrt(h)
    Qc_bias = diag([sigma_bV_rw^2, sigma_bDO_rw^2]);

    Qd = blkdiag(Qc_phys, Qc_bias) * dt;
    ...
    % ---------- measurement models with additive sensor bias ----------
    H_V   = [1 0 0 0 0 0 1 0];
    ...
    % Augmented linear model: physical dynamics + random-walk biases
    A_aug = zeros(8,8);
    A_aug(1:6,1:6) = A;

    Ad = expm(A_aug * dt);

    % Bias states remain unchanged in mean prediction
    xhat_aug = [x_phys; bV; bDO];
    ...
    K = zeros(8, max(1,size(Hk,1)));  % safe for logging

    if ~isempty(Hk)
        Rk = diag(rdiag);

        innov = yk - Hk * xhat_aug;
        S = Hk * P * Hk' + Rk;
        S = 0.5 * (S + S');

        K = (P * Hk') / S;

        xhat_aug = xhat_aug + K * innov;
    ...
    if mev_included
        ...
        % [t_h, y_mev, xpred_mev, xpost_mev, innov_mev, P55_pred, K55_mev, nis, bV_hat, bDO_hat]
        K55_mev = K(5, idx_mev);
        assay_row = [t, y(4), xhat_pred(5), xhat_aug(5), innov_mev, ...
                     P_pred(5,5), K55_mev, nis, xhat_aug(7), xhat_aug(8)];
    ...
    assignin('base', 'L1_bias_est', [xhat_aug(7), xhat_aug(8)]);

    % ---------- save back ----------
    block.Dwork(1).Data  = xhat_aug;
